...
namespace Final1.Tests
{
    public abstract class Final1TestBase : AbpIntegratedTestBase<Final1TestModule>
    {
        ...
        protected void UsingDbContext(Action<Final1DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Final1DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Final1DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Final1DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Final1DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Final1DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Final1DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Final1DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Final1DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Final1DbContext, Task<T>> func)
        {
            T result;
